...
Full guide: fixmyskills `agent-orchestration` → [claude-code.md](.agents/skills/agent-orchestration/references/claude-code.md).
...
- Prefix every model run (subagents, workflows, delegated `cursor-agent` calls) with the model name/id, e.g. `[fable-5]`, `[composer-2.5]`, `[opus-4.8]`, so which model produced which output is visible in the UI.
